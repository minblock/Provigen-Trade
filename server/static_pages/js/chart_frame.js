'use strict';
 ...
 function onOpenSocket()
 {
   ...
   setInterval(() => {  socket.send(JSON.stringify({request: 'getchart', message: [utils.MAIN_COIN, g_CurrentPair, g_currentChartPeriod]}));}, 1000)
 }
 ...
 function SetChartLegend()
 {
   ...
   $.getJSON( "/api/v1/public/getmarketsummary?market="+BTC+"-"+COIN+"&period="+g_currentChartPeriod, ret => {
     if (!ret || !ret.success || ret.success != true || BTC != coinNameToTicker[utils.MAIN_COIN].ticker || COIN != coinNameToTicker[g_CurrentPair].ticker) 
       return;
	...
	let iswdisabled = '';
     ...
     const legend = $(
       ...
       '<ul class="nav" style="padding-bottom: 5px; display:inline-block;" >'+
         ...
         '<li class="nav-item ml-2" style="display:inline-block;"><h4>'+BTC+' / '+COIN+'</h4></li>'+
	 ...
	 '</ul></div><div class="col-xs-9" style="display: inline-block; vertical-align: top;padding-left:20px;"><div class="row"><div class="col-xs-6 mobchartstats" style="display: inline-block;"><ul class="nav" style="padding-bottom: 5px; display:block;" >'+
         ...
         '<li class="nav-item ml-2">'+group+'Vol: '+(ret.result.BTCVolume*1).toFixed(8)+' '+BTC+'</li>'+
         '</ul></div></div></div></div>'
       )//('<h4>'+COIN+' / '+BTC+'</h4>');
     $('#chart_legend').empty();
...
//      	featuredmarket =$('<img src="https://i.imgur.com/6zVrpJn.png">');
//     }
